<template>
  <div>
    <v-toolbar color="white">
      <v-toolbar-title>TN_GOOD_USER 이관</v-toolbar-title>
      <v-spacer></v-spacer>
    </v-toolbar>
            ...
            <v-card-text>
              <v-container grid-list-md>
                ...
                <v-layout wrap>
                  ...
                  <v-flex xs12 md12>
                    <span style="color: purple">** 필요없는 고객정보는 엑셀로 저장하여 CRM 담당자에게 삭제요청 해주시기 바랍니다.</span>
                    <br />
                  </v-flex>
                </v-layout>
                <v-layout wrap>
                        ...
                        <table>
                          <thead>
                            <tr>
                              ...
                              <th>TELNO</th>
                              <th>JOB_CDS</th>
                              <th>JOB_COUNTERPART_CDS</th>
                              <th>INVT_GOODS_CDS</th>
                              <th>WORK_DEPT</th>
                            </tr>
                          </thead>
                          <tbody>
                            <tr v-for="(item, index) in itemList" :key="index">
                              ...
                              <td>{{ item.TELNO }}</td>
                              <td>{{ item.JOB_CDS }}</td>
                              <td>{{ item.JOB_COUNTERPART_CDS }}</td>
                              <td>{{ item.INVT_GOODS_CDS }}</td>
...
export default {
  ...
  methods: {
    getList: function () {
      ...
      console.log('getList')
      axios
        .get(Config.base_url + '/api/datamanage/crm/gettngoodsuserlist', {
          params: {},
        })
    ...
    deleteList: function () {
      ...
      axios.post(Config.base_url + '/api/datamanage/crm/deletetngoodsuserlist', {}).then(function (response) {
    ...
    moveList: function () {
      ...
      axios.post(Config.base_url + '/api/datamanage/crm/movetngoodsuserlist', {}).then(function (response) {
        // console.log(response);
        if (response.data.success == false) {
          alert(response.data.message)
        } else {
