...
module.exports = class {
	constructor(dbinfo) {
		...
		this.pool_kccrm = mysql.createPool(dbinfo);
	}

	connect() {
		return this.pool_kccrm.getConnectionAsync().disposer((conn) => {
			return conn.release();
		});
	}

	end() {
		this.pool_kccrm.end(function (err) {
	...
	getMapper(dirLevel) {
		...
		ibatisMapper.createMapper([dirPrefix + "database/mysql/crm/tnGoods.xml"]);
