...
function createCustomLayout(colorCodeFn) {
  return function (config) {
    return function (logEvent) {
      ...
      const logMessage = `${colorCode}[${timestamp}] ${logLevel} - \x1b[0m(id:${instanceId}) ${logEvent.data[0]}`;
      return logMessage;
    };
  };
}
...
log4js.configure({
	appenders: {
		...
		output: { type: "dateFile", pattern: "yyyy-MM-dd.log", alwaysIncludePattern: true, filename: "/data/koscom/kc_checktool/server/logs/CHECKTOOL",  layout: { type: 'custom' } },
	},
	categories: {
